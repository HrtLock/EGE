...
**Тип заданий 2**<br>
В текстовом файле 24.txt находится цепочка из символов латинского алфавита A, B, C, D, E, F. Найдите длину самой длинной подцепочки, состоящей из символов A, B, E, F (в произвольном порядке).
```python
f = open('24.txt')
a = list(map(str, f.read().split()))[0]
max = 0
c = 0
for i in a:
...
**Тип заданий 3**<br>
Текстовый файл 24.txt состоит не более чем из 106 символов F, A, I, L. Определите максимальное количество подряд идущих одинаковых букв. 
```python
f = open('24.txt')
a = list(map(str, f.read().split()))[0]
max = 0
